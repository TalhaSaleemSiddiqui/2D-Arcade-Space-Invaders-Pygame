...
asteroid = pygame.image.load("asteroid50.png")
...
def main():
    ...
    playagain = font.render(
        "Play Again. Press S to Restart", 1, (255, 255, 255))
            ...
            # if the player does not have sheild then decrease health and check if the player has lives left
                ...
                if health_bar.decrease_health():
                    ...
                    if lives == 0:
                        ...
                        win.blit(playagain, (SCREEN_WIDTH // 2 - 200, SCREEN_HEIGHT // 2 - 50))
...
while run:
    ...
    if not game_over:
        ...
        # random axis to spawn asteriods
        random_point = (random.randint(0, SCREEN_WIDTH),
                        random.randint(0, SCREEN_HEIGHT))

        if count % 50 == 0:
            asteriods.append(
                Asteroid(random_point[0], random_point[1], asteroid))
